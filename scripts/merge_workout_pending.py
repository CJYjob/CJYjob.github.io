...
def validate(records: list[dict[str, Any]]) -> None:
    ...
    for i, r in enumerate(records):
        ...
        started = datetime.fromisoformat(str(r["started_at"])); ended = datetime.fromisoformat(str(r["ended_at"]))
...
def correct(records: list[dict[str, Any]]) -> int:
    changed = 0
    for r in records:
        if (r.get("date") == "2026-06-08" and r.get("session_id") == "2026-06-08-evening" and r.get("exercise") == "Machine Arm Curl" and r.get("started_at") == "2026-06-08T20:16:31+09:00" and r.get("ended_at") == "2026-06-08T20:21:11+09:00"):
            r["exercise"] = "Straight-Arm Pulldown"; r["target_muscles"] = ["Latissimus Dorsi"]
            if append_note_once(r, "Corrected from Machine Arm Curl / Biceps Brachii to Straight-Arm Pulldown / Latissimus Dorsi."):
                changed += 1
        if (r.get("date") == "2026-06-14" and r.get("session_id") == "2026-06-14-afternoon" and r.get("exercise") == "Squat" and r.get("target_muscles") != ["Lower Body Muscles"]):
            r["target_muscles"] = ["Lower Body Muscles"]
            if append_note_once(r, "Target muscle corrected to Lower Body Muscles."):
                changed += 1
        if (r.get("date") == "2026-06-23" and r.get("session_id") == "2026-06-23-morning" and r.get("exercise") == "Assisted Chin-Up" and r.get("target_muscles") != ["Latissimus Dorsi"]):
            r["target_muscles"] = ["Latissimus Dorsi"]
            if append_note_once(r, "Target muscle corrected to Latissimus Dorsi only based on user approval."):
                changed += 1
    return changed


def merge(records: list[dict[str, Any]], pending: list[dict[str, Any]]) -> int:
    seen = {key(r) for r in records}; added = 0
    for r in pending:
        if key(r) in seen:
            continue
        records.append(r); seen.add(key(r)); added += 1
...
def stats(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"latest": latest, "count": len(records), "sessions": len({r["session_id"] for r in records}), "duration": round(sum(float(r.get("duration_min", 0)) for r in records), 2), "distance": round(sum(float(r.get("distance_km", 0)) for r in records if r.get("type") == "cardio"), 2), "volume": round(sum(float(s.get("volume_kg", 0)) for r in records if r.get("type") == "strength" for s in r.get("sets", [])), 2)}


def render(records: list[dict[str, Any]]) -> str:
    s = stats(records); latest = [r for r in records if r["date"] == s["latest"]]
...
def main() -> None:
    records = load(WORKOUT); corrections = correct(records)
    pending_paths = sorted(DATA.glob("workout_pending_*.json")); pending: list[dict[str, Any]] = []
    for p in pending_paths:
        pending.extend(load(p))
    validate(records)
    if pending:
        validate(pending)
    added = merge(records, pending) if pending else 0; validate(records)
    if corrections or added:
        save(WORKOUT, records); PAGE.write_text(render(records), encoding="utf-8")
    ...
    for p in pending_paths:
        p.unlink(); print(f"Removed pending file: {p.relative_to(ROOT)}")
